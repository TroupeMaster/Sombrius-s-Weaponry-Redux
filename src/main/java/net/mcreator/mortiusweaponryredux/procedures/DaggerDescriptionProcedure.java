...
public class DaggerDescriptionProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, ItemStack itemstack, List<Component> tooltip) {
		...
		if (itemstack.is(ItemTags.create(new ResourceLocation("mortius_weaponry_redux:dagger")))) {
			...
			tooltip.add(Component.literal((" \u00A72" + Component.translatable("item.mortius_weaponry_redux.dagger.special1").getString())));
		}
		if (itemstack.getItem() == MortiusWeaponryReduxModItems.TRAITORS_DAGGER.get()) {
			...
			if (Screen.hasShiftDown()) {
				...
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.traitors_dagger.desc").getString())));
			} else {
