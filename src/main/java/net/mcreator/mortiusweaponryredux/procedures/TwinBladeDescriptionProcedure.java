...
public class TwinBladeDescriptionProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, ItemStack itemstack, List<Component> tooltip) {
		if (tooltip == null)
			return;
		if (itemstack.is(ItemTags.create(new ResourceLocation("mortius_weaponry_redux:twinblade")))) {
			...
			tooltip.add(Component.literal((" \u00A72" + Component.translatable("item.mortius_weaponry_redux.twin_blade.special1").getString())));
		}
		if (itemstack.getItem() == MortiusWeaponryReduxModItems.OPPOSING_WINDS.get()) {
			tooltip.add(Component.literal(""));
			if (Screen.hasShiftDown()) {
				tooltip.add(Component.literal(("\u00A76" + Component.translatable("item.mortius_weaponry_redux.special_ability").getString())));
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.opposing_winds.desc").getString())));
			} else {
