...
public class PikeDescriptionProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, ItemStack itemstack, List<Component> tooltip) {
		...
		if (itemstack.getItem() == MortiusWeaponryReduxModItems.KRAKEN_HUNTER.get()) {
			...
			if (Screen.hasShiftDown()) {
				...
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.kraken_hunter.desc1").getString())));
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.kraken_hunter.desc2").getString())));
			} else {
