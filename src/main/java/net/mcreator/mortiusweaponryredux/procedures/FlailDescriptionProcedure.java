...
public class FlailDescriptionProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, ItemStack itemstack, List<Component> tooltip) {
		...
		if (itemstack.is(ItemTags.create(new ResourceLocation("mortius_weaponry_redux:flail")))) {
			...
			tooltip.add(Component.literal((" \u00A72" + Component.translatable("item.mortius_weaponry_redux.flail.special1").getString())));
		}
		if (itemstack.getItem() == MortiusWeaponryReduxModItems.SUN_FURY.get()) {
			tooltip.add(Component.literal(""));
			if (Screen.hasShiftDown()) {
				tooltip.add(Component.literal(("\u00A76" + Component.translatable("item.mortius_weaponry_redux.special_ability").getString())));
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.sun_fury.desc1").getString())));
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.sun_fury.desc2").getString())));
			} else {
