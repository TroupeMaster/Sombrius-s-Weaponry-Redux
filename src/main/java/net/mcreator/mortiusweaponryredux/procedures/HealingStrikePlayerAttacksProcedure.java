...
public class HealingStrikePlayerAttacksProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity sourceentity, double amount) {
		if (sourceentity == null)
			return;
		if (sourceentity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(MortiusWeaponryReduxModMobEffects.HEALING_STRIKE.get())) {
			if (sourceentity instanceof LivingEntity _entity)
				_entity.setHealth((float) ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) + Math.round(amount / 5.5)));
		}
	}
}
