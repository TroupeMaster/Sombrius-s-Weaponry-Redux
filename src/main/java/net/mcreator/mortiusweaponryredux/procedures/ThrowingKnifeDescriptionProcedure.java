...
public class ThrowingKnifeDescriptionProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, ItemStack itemstack, List<Component> tooltip) {
		...
		if (itemstack.getItem() == MortiusWeaponryReduxModItems.OBSIDIAN_KNIFE.get()) {
			...
			if (Screen.hasShiftDown()) {
				...
				tooltip.add(Component.literal((" \u00A7a-" + Component.translatable("item.mortius_weaponry_redux.obsidian_knife.desc").getString())));
			} else {
