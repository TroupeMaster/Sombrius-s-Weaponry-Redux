...
public class IronArrowEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(Items.ARROW);

	public IronArrowEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(MortiusWeaponryReduxModEntities.IRON_ARROW.get(), world);
	}
	...
	public static IronArrowEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		IronArrowEntity entityarrow = new IronArrowEntity(MortiusWeaponryReduxModEntities.IRON_ARROW.get(), entity, world);
	...
	public static IronArrowEntity shoot(LivingEntity entity, LivingEntity target) {
		IronArrowEntity entityarrow = new IronArrowEntity(MortiusWeaponryReduxModEntities.IRON_ARROW.get(), entity, entity.level());
		double dx = target.getX() - entity.getX();
		double dy = target.getY() + target.getEyeHeight() - 1.1;
		double dz = target.getZ() - entity.getZ();
		entityarrow.shoot(dx, dy - entityarrow.getY() + Math.hypot(dx, dz) * 0.2F, dz, 1.9f * 2, 12.0F);
		entityarrow.setSilent(true);
